...
if (isset($_POST['submit_review'])) {
    ...
    if (!empty($_POST["rating"]) && !empty($_POST["review_content"])) {
        ...
        // user_id বের করা
        $userResult = mysqli_query($conn, "SELECT id_user FROM users WHERE username = '$username'");
        $userRow = mysqli_fetch_assoc($userResult);
        $user_id = $userRow['id_user'];

        // Review Insert করা
        $sql = "INSERT INTO review (prod_id, user_id, rate, commentaire, review_date) 
                VALUES ('$productid', '$user_id', '$rate', '$review_content', '$date')";
        $result = mysqli_query($conn, $sql);

        // Product টেবিলে নতুন rate update করা
        $avgSql = "SELECT AVG(rate) as avgRate FROM review WHERE prod_id = '$productid'";
        $avgResult = mysqli_query($conn, $avgSql);
        $avgRow = mysqli_fetch_assoc($avgResult);
        $newRate = round($avgRow['avgRate']);

        mysqli_query($conn, "UPDATE producttb SET product_rate = '$newRate' WHERE productid = '$productid'");

        if ($result) {
            echo '<script>alert("Your review added successfully!");</script>';
            echo '<script>window.location="product.php?id='.$productid.'"</script>';
        } else {
            echo '<script>alert("Error: Review not added.");</script>';
        }
    } else {
        echo '<script>alert("Please fill all the fields (rating & review).")</script>';
        echo '<script>window.location="product.php?id='.$_GET["id"].'"</script>';
    }
}


// ---- User Activity Log with Category ----
if (isset($_SESSION['username'])) {
    $username = $_SESSION['username'];
    $productid = $_GET["id"];

    // get category of this product
    $catResult = mysqli_query($conn, "SELECT product_category FROM producttb WHERE productid = '$productid'");
    $catRow = mysqli_fetch_assoc($catResult);
    $category_id = $catRow['product_category'];

    // insert user activity
    $sql = "INSERT INTO user_activity (user_id, category_id, product_id, activity_type, activity_time)
            VALUES (
	...
	<body>
								...
								<div id="tab1" class="tab-pane active">
									<div class="products-slick" data-nav="#slick-nav-1">
										...
										<?php
												...
												while ($row = mysqli_fetch_assoc($result)){	
													component($row['productid'],$row['product_name'], $row['productOldPrice'],$row['productNewPrice'], $row['product_image'],$row['category_name'] ,$row['product_rate'], $curPageName, "Related");
												}
										?>
										<!-- /product -->
									</div>
		...
		<script>
			...
			window.addEventListener('beforeunload', function() {
				...
				let duration = Math.round((endTime - startTime) / 1000); // সময় সেকেন্ডে
				let productId = "<?php echo $productid; ?>"; // product.php তে যেই product id আছে

				// AJAX দিয়ে view time পাঠানো
				navigator.sendBeacon('store_view_time.php', new URLSearchParams({
